...
# What the deployer is allowed to DO — starts as just "push images".
# Future pipeline phases (deploying Cloud Run, running Terraform in CI)
# add their own grants here as they land, same grow-as-needed pattern as
# the runtime SA's per-secret grants in cloud_run.tf.
